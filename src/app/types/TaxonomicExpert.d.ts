/* eslint-disable */
/**
 * This file was automatically generated by json-schema-to-typescript.
 * DO NOT MODIFY IT BY HAND. Instead, modify the source JSONSchema file,
 * and run json-schema-to-typescript to regenerate this file.
 */

/**
 * A Taxonomic Expert is a person who is an expert in the field of taxonomy, the science of defining and naming groups of biological organisms on the basis of shared characteristics.
 */
export interface TaxonomicExpert {
  /**
   * The unique identifier of the Taxonomic Expert object
   */
  "@id": string;
  /**
   * The type of the object, in this case TaxonomicExpert
   */
  "@type"?: "TaxonomicExpert";
  /**
   * The date on which the Taxonomic Expert was created or the item was added to a DataFeed, following the ISO Date Time Format yyyy-MM-dd'T'HH:mm:ss.SSSXXX
   */
  "schema:dateCreated"?: string;
  /**
   * The date on which the Expert was most recently modified or when the item's entry was modified within a DataFeed, following the ISO Date Time Format yyyy-MM-dd'T'HH:mm:ss.SSSXXX
   */
  "schema:dateModified"?: string;
  /**
   * Date of the expert was published, following the ISO Date Time Format yyyy-MM-dd'T'HH:mm:ss.SSSXXX
   */
  "schema:datePublished"?: string;
  /**
   * The status of the record
   */
  "schema:status"?: "proposed" | "accepted" | "rejected";
  "schema:person"?: {
    /**
     * The name of the Taxonomic Expert
     */
    "schema:name"?: string;
    /**
     * The email of the Taxonomic Expert
     */
    "schema:email"?: string;
    /**
     * A unique identifier to identify the author; ORCID identifiers are valid
     */
    "schema:identifier"?: string;
    /**
     * A short description of the Taxonomic Expert
     */
    "schema:headline"?: string;
    /**
     * Select your nationality. Please use the country name and ISO code (Alpha-2 code).
     */
    "schema:nationality"?:
      | "Afghanistan (AF)"
      | "Albania (AL)"
      | "Algeria (DZ)"
      | "American Samoa (AS)"
      | "Andorra (AD)"
      | "Angola (AO)"
      | "Antigua and Barbuda (AG)"
      | "Argentina (AR)"
      | "Armenia (AM)"
      | "Australia (AU)"
      | "Austria (AT)"
      | "Azerbaijan (AZ)"
      | "Bahamas (BS)"
      | "Bahrain (BH)"
      | "Bangladesh (BD)"
      | "Barbados (BB)"
      | "Belarus (BY)"
      | "Belgium (BE)"
      | "Belize (BZ)"
      | "Benin (BJ)"
      | "Bhutan (BT)"
      | "Bolivia (BO)"
      | "Bosnia and Herzegovina (BA)"
      | "Botswana (BW)"
      | "Brazil (BR)"
      | "Brunei (BN)"
      | "Bulgaria (BG)"
      | "Burkina Faso (BF)"
      | "Burundi (BI)"
      | "Cambodia (KH)"
      | "Cameroon (CM)"
      | "Canada (CA)"
      | "Cape Verde (CV)"
      | "Central African Republic (CF)"
      | "Chad (TD)"
      | "Chile (CL)"
      | "China (CN)"
      | "Colombia (CO)"
      | "Comoros (KM)"
      | "Congo (Brazzaville) (CG)"
      | "Congo (Kinshasa) (CD)"
      | "Costa Rica (CR)"
      | "Côte d'Ivoire (CI)"
      | "Croatia (HR)"
      | "Cuba (CU)"
      | "Cyprus (CY)"
      | "Czech Republic (CZ)"
      | "Denmark (DK)"
      | "Djibouti (DJ)"
      | "Dominica (DM)"
      | "Dominican Republic (DO)"
      | "Ecuador (EC)"
      | "Egypt (EG)"
      | "El Salvador (SV)"
      | "Equatorial Guinea (GQ)"
      | "Eritrea (ER)"
      | "Estonia (EE)"
      | "Eswatini (SZ)"
      | "Ethiopia (ET)"
      | "Fiji (FJ)"
      | "Finland (FI)"
      | "France (FR)"
      | "Gabon (GA)"
      | "Gambia (GM)"
      | "Georgia (GE)"
      | "Germany (DE)"
      | "Ghana (GH)"
      | "Greece (GR)"
      | "Grenada (GD)"
      | "Guatemala (GT)"
      | "Guinea (GN)"
      | "Guinea-Bissau (GW)"
      | "Guyana (GY)"
      | "Haiti (HT)"
      | "Honduras (HN)"
      | "Hungary (HU)"
      | "Iceland (IS)"
      | "India (IN)"
      | "Indonesia (ID)"
      | "Iran (IR)"
      | "Iraq (IQ)"
      | "Ireland (IE)"
      | "Israel (IL)"
      | "Italy (IT)"
      | "Jamaica (JM)"
      | "Japan (JP)"
      | "Jordan (JO)"
      | "Kazakhstan (KZ)"
      | "Kenya (KE)"
      | "Kiribati (KI)"
      | "North Korea (KP)"
      | "South Korea (KR)"
      | "Kuwait (KW)"
      | "Kyrgyzstan (KG)"
      | "Laos (LA)"
      | "Latvia (LV)"
      | "Lebanon (LB)"
      | "Lesotho (LS)"
      | "Liberia (LR)"
      | "Libya (LY)"
      | "Liechtenstein (LI)"
      | "Lithuania (LT)"
      | "Luxembourg (LU)"
      | "Madagascar (MG)"
      | "Malawi (MW)"
      | "Malaysia (MY)"
      | "Maldives (MV)"
      | "Mali (ML)"
      | "Malta (MT)"
      | "Marshall Islands (MH)"
      | "Mauritania (MR)"
      | "Mauritius (MU)"
      | "Mexico (MX)"
      | "Micronesia (FM)"
      | "Moldova (MD)"
      | "Monaco (MC)"
      | "Mongolia (MN)"
      | "Montenegro (ME)"
      | "Morocco (MA)"
      | "Mozambique (MZ)"
      | "Myanmar (MM)"
      | "Namibia (NA)"
      | "Nauru (NR)"
      | "Nepal (NP)"
      | "Netherlands (NL)"
      | "New Zealand (NZ)"
      | "Nicaragua (NI)"
      | "Niger (NE)"
      | "Nigeria (NG)"
      | "North Macedonia (MK)"
      | "Norway (NO)"
      | "Oman (OM)"
      | "Pakistan (PK)"
      | "Palau (PW)"
      | "Palestine (PS)"
      | "Panama (PA)"
      | "Papua New Guinea (PG)"
      | "Paraguay (PY)"
      | "Peru (PE)"
      | "Philippines (PH)"
      | "Poland (PL)"
      | "Portugal (PT)"
      | "Qatar (QA)"
      | "Romania (RO)"
      | "Russia (RU)"
      | "Rwanda (RW)"
      | "Saint Kitts and Nevis (KN)"
      | "Saint Lucia (LC)"
      | "Saint Vincent and the Grenadines (VC)"
      | "Samoa (WS)"
      | "San Marino (SM)"
      | "Sao Tome and Principe (ST)"
      | "Saudi Arabia (SA)"
      | "Senegal (SN)"
      | "Serbia (RS)"
      | "Seychelles (SC)"
      | "Sierra Leone (SL)"
      | "Singapore (SG)"
      | "Slovakia (SK)"
      | "Slovenia (SI)"
      | "Solomon Islands (SB)"
      | "Somalia (SO)"
      | "South Africa (ZA)"
      | "South Sudan (SS)"
      | "Spain (ES)"
      | "Sri Lanka (LK)"
      | "Sudan (SD)"
      | "Suriname (SR)"
      | "Sweden (SE)"
      | "Switzerland (CH)"
      | "Syria (SY)"
      | "Taiwan (TW)"
      | "Tajikistan (TJ)"
      | "Tanzania (TZ)"
      | "Thailand (TH)"
      | "Timor-Leste (TL)"
      | "Togo (TG)"
      | "Tonga (TO)"
      | "Trinidad and Tobago (TT)"
      | "Tunisia (TN)"
      | "Turkey (TR)"
      | "Turkmenistan (TM)"
      | "Tuvalu (TV)"
      | "Uganda (UG)"
      | "Ukraine (UA)"
      | "United Arab Emirates (AE)"
      | "United Kingdom (GB)"
      | "United States (US)"
      | "Uruguay (UY)"
      | "Uzbekistan (UZ)"
      | "Vanuatu (VU)"
      | "Vatican City (VA)"
      | "Venezuela (VE)"
      | "Vietnam (VN)"
      | "Yemen (YE)"
      | "Zambia (ZM)"
      | "Zimbabwe (ZW)";
    /**
     * Select your country of residence. Please use the country name and ISO code (Alpha-2 code).
     */
    "schema:location"?:
      | "Afghanistan (AF)"
      | "Albania (AL)"
      | "Algeria (DZ)"
      | "American Samoa (AS)"
      | "Andorra (AD)"
      | "Angola (AO)"
      | "Antigua and Barbuda (AG)"
      | "Argentina (AR)"
      | "Armenia (AM)"
      | "Australia (AU)"
      | "Austria (AT)"
      | "Azerbaijan (AZ)"
      | "Bahamas (BS)"
      | "Bahrain (BH)"
      | "Bangladesh (BD)"
      | "Barbados (BB)"
      | "Belarus (BY)"
      | "Belgium (BE)"
      | "Belize (BZ)"
      | "Benin (BJ)"
      | "Bhutan (BT)"
      | "Bolivia (BO)"
      | "Bosnia and Herzegovina (BA)"
      | "Botswana (BW)"
      | "Brazil (BR)"
      | "Brunei (BN)"
      | "Bulgaria (BG)"
      | "Burkina Faso (BF)"
      | "Burundi (BI)"
      | "Cambodia (KH)"
      | "Cameroon (CM)"
      | "Canada (CA)"
      | "Cape Verde (CV)"
      | "Central African Republic (CF)"
      | "Chad (TD)"
      | "Chile (CL)"
      | "China (CN)"
      | "Colombia (CO)"
      | "Comoros (KM)"
      | "Congo (Brazzaville) (CG)"
      | "Congo (Kinshasa) (CD)"
      | "Costa Rica (CR)"
      | "Côte d'Ivoire (CI)"
      | "Croatia (HR)"
      | "Cuba (CU)"
      | "Cyprus (CY)"
      | "Czech Republic (CZ)"
      | "Denmark (DK)"
      | "Djibouti (DJ)"
      | "Dominica (DM)"
      | "Dominican Republic (DO)"
      | "Ecuador (EC)"
      | "Egypt (EG)"
      | "El Salvador (SV)"
      | "Equatorial Guinea (GQ)"
      | "Eritrea (ER)"
      | "Estonia (EE)"
      | "Eswatini (SZ)"
      | "Ethiopia (ET)"
      | "Fiji (FJ)"
      | "Finland (FI)"
      | "France (FR)"
      | "Gabon (GA)"
      | "Gambia (GM)"
      | "Georgia (GE)"
      | "Germany (DE)"
      | "Ghana (GH)"
      | "Greece (GR)"
      | "Grenada (GD)"
      | "Guatemala (GT)"
      | "Guinea (GN)"
      | "Guinea-Bissau (GW)"
      | "Guyana (GY)"
      | "Haiti (HT)"
      | "Honduras (HN)"
      | "Hungary (HU)"
      | "Iceland (IS)"
      | "India (IN)"
      | "Indonesia (ID)"
      | "Iran (IR)"
      | "Iraq (IQ)"
      | "Ireland (IE)"
      | "Israel (IL)"
      | "Italy (IT)"
      | "Jamaica (JM)"
      | "Japan (JP)"
      | "Jordan (JO)"
      | "Kazakhstan (KZ)"
      | "Kenya (KE)"
      | "Kiribati (KI)"
      | "North Korea (KP)"
      | "South Korea (KR)"
      | "Kuwait (KW)"
      | "Kyrgyzstan (KG)"
      | "Laos (LA)"
      | "Latvia (LV)"
      | "Lebanon (LB)"
      | "Lesotho (LS)"
      | "Liberia (LR)"
      | "Libya (LY)"
      | "Liechtenstein (LI)"
      | "Lithuania (LT)"
      | "Luxembourg (LU)"
      | "Madagascar (MG)"
      | "Malawi (MW)"
      | "Malaysia (MY)"
      | "Maldives (MV)"
      | "Mali (ML)"
      | "Malta (MT)"
      | "Marshall Islands (MH)"
      | "Mauritania (MR)"
      | "Mauritius (MU)"
      | "Mexico (MX)"
      | "Micronesia (FM)"
      | "Moldova (MD)"
      | "Monaco (MC)"
      | "Mongolia (MN)"
      | "Montenegro (ME)"
      | "Morocco (MA)"
      | "Mozambique (MZ)"
      | "Myanmar (MM)"
      | "Namibia (NA)"
      | "Nauru (NR)"
      | "Nepal (NP)"
      | "Netherlands (NL)"
      | "New Zealand (NZ)"
      | "Nicaragua (NI)"
      | "Niger (NE)"
      | "Nigeria (NG)"
      | "North Macedonia (MK)"
      | "Norway (NO)"
      | "Oman (OM)"
      | "Pakistan (PK)"
      | "Palau (PW)"
      | "Palestine (PS)"
      | "Panama (PA)"
      | "Papua New Guinea (PG)"
      | "Paraguay (PY)"
      | "Peru (PE)"
      | "Philippines (PH)"
      | "Poland (PL)"
      | "Portugal (PT)"
      | "Qatar (QA)"
      | "Romania (RO)"
      | "Russia (RU)"
      | "Rwanda (RW)"
      | "Saint Kitts and Nevis (KN)"
      | "Saint Lucia (LC)"
      | "Saint Vincent and the Grenadines (VC)"
      | "Samoa (WS)"
      | "San Marino (SM)"
      | "Sao Tome and Principe (ST)"
      | "Saudi Arabia (SA)"
      | "Senegal (SN)"
      | "Serbia (RS)"
      | "Seychelles (SC)"
      | "Sierra Leone (SL)"
      | "Singapore (SG)"
      | "Slovakia (SK)"
      | "Slovenia (SI)"
      | "Solomon Islands (SB)"
      | "Somalia (SO)"
      | "South Africa (ZA)"
      | "South Sudan (SS)"
      | "Spain (ES)"
      | "Sri Lanka (LK)"
      | "Sudan (SD)"
      | "Suriname (SR)"
      | "Sweden (SE)"
      | "Switzerland (CH)"
      | "Syria (SY)"
      | "Taiwan (TW)"
      | "Tajikistan (TJ)"
      | "Tanzania (TZ)"
      | "Thailand (TH)"
      | "Timor-Leste (TL)"
      | "Togo (TG)"
      | "Tonga (TO)"
      | "Trinidad and Tobago (TT)"
      | "Tunisia (TN)"
      | "Turkey (TR)"
      | "Turkmenistan (TM)"
      | "Tuvalu (TV)"
      | "Uganda (UG)"
      | "Ukraine (UA)"
      | "United Arab Emirates (AE)"
      | "United Kingdom (GB)"
      | "United States (US)"
      | "Uruguay (UY)"
      | "Uzbekistan (UZ)"
      | "Vanuatu (VU)"
      | "Vatican City (VA)"
      | "Venezuela (VE)"
      | "Vietnam (VN)"
      | "Yemen (YE)"
      | "Zambia (ZM)"
      | "Zimbabwe (ZW)";
    /**
     * The languages that you are proficient or fluent in. Please use one of the language codes from the IETF BCP 47 standard.
     */
    "schema:language"?: (
      | "bg"
      | "hr"
      | "cs"
      | "da"
      | "lb"
      | "nl"
      | "en"
      | "et"
      | "fi"
      | "fr"
      | "de"
      | "el"
      | "hu"
      | "ga"
      | "it"
      | "lv"
      | "lt"
      | "mt"
      | "pl"
      | "pt"
      | "ro"
      | "ru"
      | "sk"
      | "es"
      | "sv"
      | "tk"
      | "uk"
    )[];
    /**
     * The birth date of the Taxonomic Expert
     */
    "schema:birthDate"?: string;
    "schema:gender"?: string;
    "schema:affiliation"?: {
      [k: string]: unknown;
    };
    /**
     * A brief description of your expertise, skills and services.
     */
    "schema:biography"?: string;
    /**
     * Links to your personal webpage, blog, or other online profiles.
     */
    "schema:links"?: string[];
    /**
     * The URL of the Taxonomic Expert's profile picture
     */
    "schema:ProfilePicture"?: string;
    [k: string]: unknown;
  };
  "schema:Taxon"?: {
    /**
     * The taxonomic discipline that you are an expert in. For example: Entomology, Botany, Ichthyology, etc.
     */
    "schema:discipline"?: (
      | "Botany"
      | "Invertebrates Zoology"
      | "Vertebrates Zoology"
      | "Palaeontology"
      | "Microbiology"
      | "ECO Environmental DNA"
    )[];
    /**
     * The taxonomic group that you are an expert in. For example: Mammals, Birds, Insects, etc.
     */
    "schema:additionalType"?: (
      | "Algae"
      | "Macrofungi, Lichens & Myxomycetes (Mycology)"
      | "Bryophytes"
      | "Pteridophytes"
      | "Seed plants"
      | "Plant Genetic Resources"
      | "Insects"
      | "Arachnids"
      | "Crustaceans & Myriapods"
      | "Mollusca (bivalves, gastropods, cephalopods)"
      | "Porifera (Sponges)"
      | "Cnidaria (jellyfish, coral, anemones)"
      | "Echinodermata (starfish, sea urchins, sea cucumbers)"
      | "Invertebrates Genetic Resources"
      | "Other Invertebrate zoology"
      | "Fishes"
      | "Amphibians"
      | "Reptiles"
      | "Birds"
      | "Mammals"
      | "Vertebrates Genetic Resources"
      | "Other Vertebrate Zoology"
      | "Fossil Plants & Fungi"
      | "Fossil Invertebrates"
      | "Fossil Vertebrates"
      | "Other palaeontology"
      | "Phages"
      | "Bacteria & archea"
      | "Plasmids"
      | "Protozoa"
      | "Eukaryotic microorganisms"
      | "Viruses"
      | "Microfungi (including moulds, yeasts)"
      | "Algae (in microbiology collection)"
      | "Other microbiology objects (e.g. mixed or other kinds of microorganisms)"
      | "ECO-DNA (environmental DNA)"
      | "ECO-OTH"
    )[];
    /**
     * You can list in more detail the Taxonomic, orders, Families or genus you are an expert on. Please if possible write the scientific names and english common name e.g. Hymenoptera/bees
     */
    "schema:about"?: string;
    /**
     * The geographic region that you are an expert in. For example: Europe, South America, Africa, etc.
     */
    "schema:spatialCoverage"?: (
      | "Africa"
      | "Antarctica"
      | "Asia Temperate"
      | "Asia Tropical"
      | "Australasia"
      | "Europe"
      | "North America"
      | "South America"
      | "Pacific"
      | "World/NA"
      | "Arctic Ocean"
      | "Indian Ocean"
      | "North Atlantic: unknown"
      | "North Atlantic: deep sea"
      | "North Atlantic: shelf area & adjacent sea"
      | "South Atlantic: unknown"
      | "South Atlantic: deep sea"
      | "South Atlantic: shelf area & adjacent seas"
      | "North Pacific: unknown"
      | "North Pacific: deep sea"
      | "North Pacific: shelf area & adjacent seas"
      | "South Pacific: unknown"
      | "South Pacific: deep sea"
      | "South Pacific: shelf area & adjacent seas"
      | "Southern Ocean"
      | "World Marine"
    )[];
    /**
     * The stratigraphy age that you are an expert in. For example: Cretaceous, Jurassic, etc.
     */
    "schema:temporalCoverage"?: string;
    /**
     * The taxonomic methodology that you are an expert in. For example: DNA barcoding, Morphology, etc.
     */
    "schema:measurementTechnique"?: (
      | "Morphological"
      | "Molecular/genomica"
      | "bioacoustic"
      | "Integrative"
      | "cladistics"
      | "faunistics"
      | "other"
    )[];
    /**
     * Keywords that describe your expertise. For example: Taxonomy, Biodiversity, Conservation, etc.
     */
    "schema:ResearchProject"?: (
      | "100136 - environmental biotechnology"
      | "100180 - environmental engineering"
      | "100381 - environmental sciences"
      | "100382 - meteorology"
      | "100391 - natural sciences"
      | "100408 - environmental geography"
      | "100409 - geography"
      | "100549 - environmental impact assessment"
      | "101045 - environmental chemistry"
      | "101070 - climate change"
      | "101078 - applied environmental sciences"
      | "101317 - environmental and public health"
      | "101318 - biodiversity conservation"
      | "101510 - environmentalism"
      | "100038 - pathobiology"
      | "100126 - biomechanics"
      | "100343 - applied biology"
      | "100345 - biological sciences"
      | "100346 - biology"
      | "100347 - ecology"
      | "100348 - environmental biology"
      | "100822 - cell biology"
      | "100826 - parasitology"
      | "100848 - aquatic biology"
      | "100849 - freshwater biology"
      | "100850 - population biology"
      | "100858 - evolution"
      | "100864 - ecosystem ecology and land use"
      | "100865 - biometry"
      | "100869 - bioinformatics"
      | "100949 - biophysical science"
      | "101210 - biomaterials"
      | "101352 - biogeography"
      | "101457 - community ecology"
      | "101458 - population ecology"
      | "100264 - anatomy"
      | "100356 - zoology"
      | "100522 - animal behaviour"
      | "100880 - applied zoology"
      | "100881 - cell zoology"
      | "100882 - entomology"
      | "100883 - marine zoology"
      | "100884 - pest management"
      | "100937 - animal physiology"
      | "100940 - animal nutrition"
      | "100139 - plant biotechnology"
      | "100355 - plant sciences"
      | "100520 - forestry and arboriculture"
      | "100529 - horticulture"
      | "100872 - mycology"
      | "100873 - plant cell science"
      | "100874 - plant pathology"
      | "100932 - plant biochemistry"
      | "100945 - crop protection"
      | "100946 - crop nutrition"
      | "100947 - crop production"
      | "101024 - agricultural chemistry"
      | "101025 - agricultural botany"
      | "101348 - exotic plants and crops"
      | "101376 - applied botany"
      | "101460 - plant physiology"
      | "100353 - microbiology"
      | "100906 - applied microbiology"
      | "100909 - bacteriology"
      | "100910 - virology"
      | "marineScience - Marine science"
      | "100351 - marine biology"
      | "100418 - marine sciences"
      | "100421 - ocean sciences"
      | "100544 - marine engineering"
      | "101046 - marine chemistry"
      | "101065 - maritime geography"
      | "101390 - marine physics"
      | "100137 - industrial biotechnology"
      | "100141 - biochemical engineering"
      | "100259 - genetics"
      | "100344 - biochemistry"
      | "100354 - molecular biology"
      | "100900 - molecular genetics"
      | "100901 - genomics"
      | "100902 - population genetics"
      | "100948 - biomolecular science"
      | "100003 - ceramics"
      | "100033 - metallurgy"
      | "100298 - ancient history"
      | "100299 - archaeology"
      | "100300 - classical studies"
      | "100302 - history"
      | "100307 - history of science"
      | "100309 - medieval history"
      | "100310 - modern history"
      | "100312 - social history"
      | "100314 - humanities"
      | "100350 - human biology"
      | "100384 - archaeological sciences"
      | "100386 - forensic biology"
      | "100436 - anthropology"
      | "100437 - social anthropology"
      | "100478 - human geography"
      | "100663 - physical and biological anthropology"
      | "100787 - Egyptology"
      | "100898 - human genetics"
      | "100918 - museum studies"
      | "101218 - forensic anthropology"
      | "101219 - forensic archaeology"
      | "101261 - maritime archaeology"
      | "101437 - Stone Age"
      | "101438 - Bronze Age"
      | "101439 - Iron Age"
      | "101440 - classical art and archaeology"
      | "101455 - stone crafts"
      | "101394 - glaciology and cryospheric systems"
      | "101106 - engineering geology"
      | "101105 - petroleum geology"
      | "101104 - applied geology"
      | "101103 - planetary science"
      | "101102 - space science"
      | "101093 - exploration geology"
      | "101091 - quaternary studies"
      | "101089 - hydrogeology"
      | "101086 - geological oceanography"
      | "101084 - exploration geophysics"
      | "101083 - geochemistry"
      | "101082 - geological hazards"
      | "101081 - volcanology"
      | "101079 - hydrology"
      | "101073 - hydrography"
      | "101067 - soil science"
      | "101064 - geomorphology"
      | "101044 - crystallography"
      | "100566 - quarrying"
      | "100550 - gemmology"
      | "100424 - physical sciences"
      | "100410 - physical geography"
      | "100398 - palaeontology"
      | "100396 - geophysics"
      | "100395 - geology"
      | "100394 - earth sciences"
      | "100392 - applied science"
      | "100380 - environmental geoscience"
      | "100379 - climate science"
      | "100212 - minerals processing"
      | "100155 - minerals technology"
      | "other subjects"
    )[];
    [k: string]: unknown;
  };
  "schema:occupation"?: {
    /**
     * Select the highest level of education you have received. You can choose multiple options.
     */
    "schema:educationalLevel"?: (
      | "Practical education or apprenticeship"
      | "Undergraduate diploma or certificate"
      | "Bachelor's degree"
      | "Master's degree"
      | "PhD or doctorate"
      | "Citizen scientist training"
      | "Self-taught"
    )[];
    /**
     * Provide your current employment status. This information helps with statistical analysis and gives users of the marketplace insight into the professional experience of the expert. You can select up to two options.
     */
    "schema:employmentType"?: (
      | "Tenured professor (Full time)"
      | "Tenured professor (Part time)"
      | "Permanent researcher (Full time)"
      | "Permanent researcher (Part time)"
      | "Post-doctoral researcher (Full time)"
      | "Post-doctoral researcher (Part time)"
      | "Master student (Full time)"
      | "Master student (Part time)"
      | "Undergraduate student (Full time)"
      | "Undergraduate student (Part time)"
      | "Permanent Industry researcher (Full time)"
      | "Permanent Industry researcher (Part time)"
      | "Temporary Industry Researcher (Full time)"
      | "Temporary Industry Researcher (Part time)"
      | "Honorary researcher"
      | "Permanent curator (Full time)"
      | "Permanent curator (Part time)"
      | "Temporary curator (Full time)"
      | "Temporary curator (Part time)"
      | "Freelance taxonomist (Full time)"
      | "Freelance taxonomist (Part time)"
      | "Citizen Scientist"
      | "Retired"
    )[];
    /**
     * Select your years of experience in taxonomy. This helps gauge your professional background in the field.
     */
    "schema:yearsInOperation"?: string;
    [k: string]: unknown;
  };
  "schema:PublicationNumber"?: {
    /**
     * The name of the publication
     */
    "schema:identifier"?: number;
    /**
     * The number of scientific papers
     */
    "schema:ScholarlyArticle"?: number;
    /**
     * The number of books
     */
    "schema:Book"?: number;
    /**
     * The number of other publications
     */
    "schema:CreativeWork"?: number;
    [k: string]: unknown;
  };
  /**
   * Courses the Taxonomic Expert has created or is teaching
   */
  "schema:educationAndTrainingProvision"?: {
    /**
     * The title of the training course
     */
    "schema:name": string;
    /**
     * A brief description of the training course
     */
    "schema:description"?: string;
    /**
     * The URL of the training course
     */
    "schema:contentUrl"?: string;
    /**
     * The URL of the training course
     */
    "schema:EducationalOccupationalProgram"?: string;
    [k: string]: unknown;
  }[];
  [k: string]: unknown;
}
